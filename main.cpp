...
void makeSpline()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, splineVertices.size() * sizeof(Vertex), &splineVertices[0], GL_STATIC_DRAW);
    ...
    GLintptr offset = 0;
    glVertexAttribPointer(posLoc, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), BUFFER_OFFSET(offset));
    offset += sizeof(Vertex().position);
    glVertexAttribPointer(colorLoc, 4, GL_FLOAT, GL_FALSE, sizeof(Vertex), BUFFER_OFFSET(offset));
...
void drawSpline()
{
    glDrawArrays(GL_LINES, 0, splineVertices.size());
}
...
void makeRails()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, railVertices.size() * sizeof(Vertex), &railVertices[0], GL_STATIC_DRAW);
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, railIndices.size() * sizeof(GLuint), &railIndices[0], GL_STATIC_DRAW);
...
{
    GLintptr offset = 0;
    glDrawElements(GL_TRIANGLES, railIndices.size() / 2, GL_UNSIGNED_INT, BUFFER_OFFSET(offset));
...
glm::vec2 groundTexCoords[6] = {
    glm::vec2(0.0, 0.0),
    glm::vec2(0.0, upperLim),
    glm::vec2(upperLim, upperLim),
    glm::vec2(0.0, 0.0),
    glm::vec2(upperLim, upperLim),
    glm::vec2(upperLim, 0.0)};
...
void makeGround()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(groundVertices) + sizeof(groundTexCoords), NULL, GL_STATIC_DRAW);
    GLintptr offset = 0;
    glBufferSubData(GL_ARRAY_BUFFER, offset, sizeof(groundVertices), groundVertices);
    offset += sizeof(groundVertices);
    glBufferSubData(GL_ARRAY_BUFFER, offset, sizeof(groundTexCoords), groundTexCoords);
    ...
    offset = 0;
    glVertexAttribPointer(posLoc, 3, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(offset));
    offset += sizeof(groundVertices);
    glVertexAttribPointer(texLoc, 2, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(offset));
...
void makeSky()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(skyVertices) + sizeof(texSkyCoords), NULL, GL_STATIC_DRAW);
    GLintptr offset = 0;
...
void subdivide(GLfloat u0, GLfloat u1, GLfloat maxLineLength, glm::mat4x3 *controlMatrix)
{
...
void setupCamPath()
{
    ...
    for (int i = 0; i < splineVertices.size(); i += 1)
    {
        ...
        if (camInd == 0)
        {
            camNormals.push_back(glm::normalize(glm::cross(camTangents[i], glm::vec3(0.0, 1.0, -0.5))));
            camBinormals.push_back(glm::normalize(glm::cross(camTangents[i], camNormals[camInd])));
        }
        else
        {
            camNormals.push_back(glm::normalize(glm::cross(camBinormals[camInd - 1], camTangents[i])));
            camBinormals.push_back(glm::normalize(glm::cross(camTangents[i], camNormals[camInd])));
        }
        ++camInd;
    }
...
void setupRails()
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        {
            v[0].position = camPositions[j] + alpha * (-camNormals[j] + camBinormals[j] * 0.5f);
            v[1].position = camPositions[j] + alpha * (camBinormals[j] * 0.5f);
            v[2].position = camPositions[j] + alpha * (camBinormals[j]);
            v[3].position = camPositions[j] + alpha * (camNormals[j] + camBinormals[j]);
            v[4].position = camPositions[j] + alpha * (camNormals[j] - camBinormals[j]);
            v[5].position = camPositions[j] + alpha * (-camBinormals[j]);
            v[6].position = camPositions[j] + alpha * (-camBinormals[j] * 0.5f);
            v[7].position = camPositions[j] + alpha * (-camNormals[j] - camBinormals[j] * 0.5f);
    ...
    for (int i = 0; i < 2; ++i)
    {
        for (int j = i * railVertices.size() / 2; j < railVertices.size() / (2 - i) - num_vertices; j += num_vertices)
        {
            // top face
...
void setupCrossbars()
{
    ...
    for (int j = 1; j < camPositions.size(); ++j)
    {
        ...
        {
            v[0] = camPositions[j] + alpha * (-beta * camNormals[j] + camBinormals[j] * 0.5f) + camBinormals[j];
            v[1] = camPositions[j] + alpha * (-camNormals[j] + camBinormals[j] * 0.5f) + camBinormals[j];
            v[2] = camPositions[j] + alpha * (-beta * camNormals[j] + camBinormals[j] * 0.5f) - camBinormals[j] - alpha * camBinormals[j];
            v[3] = camPositions[j] + alpha * (-camNormals[j] + camBinormals[j] * 0.5f) - camBinormals[j] - alpha * camBinormals[j];

            v[4] = camPositions[j] + alpha * (-beta * camNormals[j] + camBinormals[j] * 0.5f) + camBinormals[j] + barDepth * camTangents[j];
            v[5] = camPositions[j] + alpha * (-camNormals[j] + camBinormals[j] * 0.5f) + camBinormals[j] + barDepth * camTangents[j];
            v[6] = camPositions[j] + alpha * (-beta * camNormals[j] + camBinormals[j] * 0.5f) - camBinormals[j] + barDepth * camTangents[j] - alpha * camBinormals[j];
            v[7] = camPositions[j] + alpha * (-camNormals[j] + camBinormals[j] * 0.5f) - camBinormals[j] + barDepth * camTangents[j] - alpha * camBinormals[j];
...
void catmullRomSpline(Spline *spline)
{
    ...
    {
        controlRows[0] = glm::vec3(spline->points[i - 1].x, spline->points[i - 1].y, spline->points[i - 1].z);
        controlRows[1] = glm::vec3(spline->points[i].x, spline->points[i].y, spline->points[i].z);
        controlRows[2] = glm::vec3(spline->points[i + 1].x, spline->points[i + 1].y, spline->points[i + 1].z);
        controlRows[3] = glm::vec3(spline->points[i + 2].x, spline->points[i + 2].y, spline->points[i + 2].z);
        controlMatrix = glm::mat4x3(controlRows[0], controlRows[1], controlRows[2], controlRows[3]);
...
void timerFunc(int val)
{
    if (val)
    {
        ...
        sprintf(temp, "%s: %d fps , %d x %d resolution",
                windowTitle, frameCount * 30, windowWidth, windowHeight);
...
void displayFunc()
{
    ...
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); // Clear back and depth buffer

    matrix->LoadIdentity();
    matrix->LookAt(camPos[0], camPos[1], camPos[2],
...
int main(int argc, char **argv)
{
    if (argc < 5)
    {
        printf("usage: %s <trackfile> <groundtexture> <skytexture> <crossbartexture>\n", argv[0]);
        exit(0);
    }
